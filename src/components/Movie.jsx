...
function Movie({ movie, remove, collection }) {
  ...
  return (
    <div className="relative p-1 bg-white rounded-sm border overflow-hidden">
      {remove && <button onClick={() => handleRemove(movie.id)} className="absolute top-3 left-[50%] -ml-5 z-10 w-10 h-10 bg-gray-900/50 rounded text-sm text-center text-red-400 leading-10"><i className="fas fa-trash"></i></button>}
